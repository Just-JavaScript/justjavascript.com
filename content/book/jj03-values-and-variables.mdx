...
If the right side must be an expression, does that mean that simple things - numbers like `2` or strings like `'The Kraken'` - written in code are also expressions? Yes! Such expressions are called *literals* — because we *literally* write down their values.
