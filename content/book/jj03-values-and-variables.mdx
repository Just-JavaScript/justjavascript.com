...
<SpoilerAlert alertTitle="Spoilers Below" alertBody="Don’t scroll further until you have finished writing." />
...
Answer: This code will either print `"yikes"` or throw an error depending on whether you are in [strict mode](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Strict_mode). **It will never print `"likes"`.**
...
This is an important detail we need to add to our mental model. A string is a primitive value, and **all primitive values are immutable.** “Immutable” is a fancy Latin way to say “unchangeable.” Read-only. We can’t mess with primitive values. At all.

JavaScript won’t let us set a property on any primitive value, be it a number, string or something else. Whether it will silently refuse our request or throw an error depends on [which mode](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Strict_mode) our code is running in. But rest assured that this will never work:
...
Like any number, `50` is a primitive value. We can't set properties on it. Remember that in our JavaScript universe, all primitive values are distant stars, floating farthest from our code. We can point to them, but they will always stay where they are, unchanged.
...
<SpoilerAlert alertTitle="Spoiler Alert" />
...
If you thought I was trying to mess with your head, you were completely right! The answer is `"The Kraken."` Immutability of strings doesn’t play a role here.
...
In our JavaScript universe, a variable is a wire connecting two things. It starts with a name in our code and it ends pointing at a value in our universe.

For example, I can point the `pet` variable at the `"Narwhal"` value. (We can also say that we're *assigning* the value `"Narwhal"` to the name `pet`):

```js
let pet = 'Narwhal';
```

![Assigning the value "Narwhal" value to the name pet](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1579283157/just-javascript-email-images/jj03/narwhal-assignment.gif)

But what if I want a different pet? No problem - I can *assign* some other value to my variable name:

```js
pet = 'The Kraken';
```

![Assigning the value "The Kraken" to the name pet](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1579283157/just-javascript-email-images/jj03/kraken-reassignment.gif)

All I am doing here is instructing JavaScript to point the “wire” from our name on the left side (`pet`) to the value on the right side (`'The Kraken'`). It will keep pointing at that value unless I re-assign it again later.

#### Rules of Assignment

There are two rules when we want to use our wire to assign a value:

1. The left side of an assignment must be a name. Note that our name can’t be a value. (Try these examples in the console):
...
2. The right side of an assignment must be an expression. Our expression can be something simple, like 2 or 'hello'. It can also be a more complicated expression — for example:

```js
pet = count + ' Dalmatians';
```

Here, `count + ' Dalmatians'` is an expression — a question to JavaScript. JavaScript will answer it with a value (for example, `"101 Dalmatians"`). From now on, the `pet` “wire” will point to that value.

// "FUN FACT" POP-OUT
If the right side must be an expression, does this mean that simple things - numbers like `2` or strings like `'The Kraken'` - written in code are also expressions? Yes! Such expressions are called *literals* — because we *literally* write down their values.
// "FUN FACT" POP-OUT
...
It turns out that a variable name like `pet` can serve as an expression too! When we write `pet`, we’re asking JavaScript a question: “What is the current value of `pet`?” To answer our question, JavaScript follows `pet`’s “wire,” and gives us back the value at the end of this “wire.”
...
But that’s not how it works. We know that `double(money)` means “figure out the *value* of `money`, and then *pass that value* to `double`.” So the answer is `10`. What a scam!

What are the different JavaScript concepts in your head? How do they relate to each other and how can we interact with them? Write down a short list of the ones you use most often.
...
At the end, the `x` variable points to the value `0`, and the `y` variable points to the value `10`. Note that `y = x` did not mean point `y` to `x`. We can’t point variables to each other! **Variables always point to *values*.** When we see an assignment, we “ask” the right side’s value, and point the left side’s “wire” at it.
...
This might seem a bit annoying, but using wires is going to be very important for explaining numerous other concepts, like strict equality, object identity, and mutation. We’re going to stick with wires, so you might as well start getting used to them now!
...
* **Variables are like wires.** A “wire” is not a JavaScript concept — but it helps us imagine how variables point to values.
...
* **Language matters.** We’re building a mental model so that we can be confident in what *can* or *cannot* happen in our universe. We might speak about these ideas in a casual way (and nitpicking is often counterproductive) but our understanding of the meaning behind the terms we use needs to be precise.
...
// Re-arrange variable assignment explanation to tighten things up. Consistently call the "left half" the name and the "right half" the value, leaving the wire connecting them in the middle. Mention expressions earlier - it's confusing that we add that rule in at the very last minute when it's pretty fundamental to the structure of a variable assignment.
