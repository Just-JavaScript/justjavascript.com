...
What is a point in geometry? What is a word in human language? A value is a fundamental concept in JavaScript — so we can’t define it through other terms. Instead, we’ll define it through examples. Numbers and strings are values. Objects and functions are values, too.
...
// "FUN FACT" POP-OUT
“I see,” you might reply, “this is because *everything* is an object!” Alas, this is a popular urban legend, but it’s not true. Although code like `"hi".toUpperCase()` makes `"hi"` seem like an object, this is nothing but an illusion.
JavaScript creates a wrapper object when you do this, and then immediately discards it. Don't let that behavior confuse you: numbers, strings, null, and other primitive types are *not* objects. It’s fine if this mechanism doesn’t quite click yet.
