...
As we start building our mental model, one of the first common misconceptions we need to clear up is that values _are_ our code. Instead, we need to think of them separately - our code interacts with values, but values exist in a completely separate space. To distinguish between values and code in my JavaScript program, I like to imagine this drawing of the Little Prince by Antoine de Saint-Exupéry:
...
All primitive values have something in common. **There’s nothing I can do in my code that would affect them.** Primitive values are like stars — cold and distant, but always there when I need them.
...
At first, all values in the JavaScript cosmos might look the same — bright dots in the sky. But if you look closely, you’ll realize there are fewer than ten different types of values: numbers, strings, objects, and so on. Values of the same type behave in similar ways, but how do we know any particular value’s type?.
...
[Click here to answer these questions](https://eggheadio.typeform.com/to/PLyTKB?email={{subscriber.email_address}}) and provide feedback about this module.
