...
What is a point in geometry? What is a word in human language? **A value is a *thing* in JavaScript.** We're using a vague word like "thing" here purposefully. Values are tricky to describe because they are self-evident - fundamental terms that require no definition. Things like numbers and strings are values. Things like objects and functions can be values, too.
...
but values exist in a completely separate space. To distinguish between values and code in my JavaScript program, I like to imagine this drawing of the Little Prince by Antoine de Saint-Exupéry:
...
I’m standing on a small planet, holding a list of instructions. This little world is my program, and this list is my code. As I read through my list of instructions, I can see a lot going on - there are `if` statements, variable declarations, commas and curly braces.

My code contains instructions like “make a function call”, “do this thing many times”, or even “throw an error”. I read through these instructions step by step, working through the code from the surface of my small planet.
...
“Hold on,“ you might say, “I always thought of values as being *inside* of my code!” Here, I’m asking you to take a leap of faith. It will take a few more modules for this mental model to pay off. [Give it five minutes.](https://signalvnoise.com/posts/3124-give-it-five-minutes)

## Values

Back to values. Broadly, there are two kinds.
...
All primitive values have something in common: **There’s nothing I can do in my code to affect them.**

To see some primitive values in practice, open your browser's console and log them:
...
**Objects and functions** are are also values but, unlike primitve values, **I can manipulate them from my code**. If primitive values are like distant stars, then objects and functions are more like satellites floating around my planet. They're not part of my code, but I can manipulate them.
...
At first, all values in the JavaScript cosmos might look the same — just bright dots in the sky. But we are here to study the stars of our JavaScript universe, so we'll need a way to categorize them.
...
### Checking a Type
There are only nine types of values, but how do we know any particular value’s type?
...
4. **We can ask questions with expressions.** JavaScript will answer us with values. For example, the `2 + 2` expression is answered with `4`.
...
We will also continue to refine our mental model. This module presents a crude sketch — an approximation. We will focus on different parts of the picture and fill them in with more details, like a [progressive JPEG](https://www.liquidweb.com/kb/what-is-a-progressive-jpeg/) image.
