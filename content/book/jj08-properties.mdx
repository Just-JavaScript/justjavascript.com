...
Importantly, properties don’t *contain* values — they *point* at them! Remember that our universe is full of wires. Some of them start in our code (variables), and others start from objects (properties). All wires always point to values.
...
Another important note is that property names are also always case-sensitive! For example, `age` and `Age` would be two completely different properties from JavaScript’s point of view.
