...
Like Virgil guided Dante through the nine circles of Hell, Count von Count will accompany us through the “celestial spheres” of JavaScript to meet different values. We'll meet the primitive values: booleans, Numbers, Strings, and so on. We'll also meet objects and functions. Consider it a sightseeing tour.
...
The next stop on our counting tour is Null, and Count von Count won't have much work to do here - **there is also only one value of this type — `null`.** You can think of `null` as `undefined`’s sister; it behaves very similarly to undefined. For example, it will also throw a fuss when you try to access its properties:
...
Next on our counting tour, booleans! Like day and night or on and off, **Count von Count will only find two boolean values: `true` and `false`.**
...
If numbers kept Count von Count busy, the next stop on our tour, BigInts, will really impress him.
...
Count von Count could be busy with numbers and BigInts for eternity, but let's keep moving. Our next tour stop is Strings, which represent text in JavaScript. There are three ways to write strings (single quotes, double quotes, and backticks), but the result is the same:
...
We've already given Count von Count plenty to keep busy, but there is just one more (quick) stop on the first part of our tour counting primitive values: Symbols. Symbols are a relatively recent addition to the language.
