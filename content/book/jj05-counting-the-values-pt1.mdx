...
You might wonder: what’s the point of chasing after values? Can't we just point to them in our list of instructions, from the comfort of our planet's surface, and get on with our lives? But getting to know the finer points of each value will give us a stronger fundamental understanding of how our JavaScript universe works. You can only say there are “two apples” when you clearly see that they’re two *distinct* apples. Distinguishing values from one another is key to understanding *equality* in JavaScript — which will be our next topic.

Our spaceship will guide us through the “celestial spheres” of JavaScript to meet different values. We'll meet the primitive values: booleans, Numbers, Strings, and so on. We'll also meet objects and functions. Consider it a sightseeing tour.

// Maybe a line or two here to really make the difference between types and values obvious?
...
The next stop on our tour is Null. You can think of `null` as `undefined`’s sister; **there is only one value of this type — `null`**. It also behaves very similarly to undefined. For example, it will also throw a fuss when you try to access its properties:
...
// "FUN FACT" POP-OUT
**`Null` is the only value of its own type** however, `null` is also a liar. Due to a [bug](https://2ality.com/2013/10/typeof-null.html) in JavaScript, it pretends to be an object:
...
Before moving continuing our tour of the JavaScript universe, let's check our mental model. Open a [sketching app](https://www.excalidraw.com/) or take a piece of paper, and sketch out the variables, the values, and the wires between them for the above snippet of code.
...
If numbers expanded our JavaScript universe, the next stop on our tour, BigInts, will really keep us busy exploring.

Regular numbers can’t represent large integers with precision, so BigInts [fill that gap](https://v8.dev/features/bigint) (literally). How many BigInts are there in our universe? The specification says they have an *arbitrary precision*. This means that **in our JavaScript universe, there is an infinite number of BigInts — one for each integer in math**.
...
Of course, in practice, we can’t fit all the possible BigInts inside the computer memory. If we tried, at some point it would crash or freeze. But conceptually, we could tour our JavaScript universe for eternity and never reach every single BigInt.
...
Our next tour stop is Strings, which represent text in JavaScript. There are three ways to write strings (single quotes, double quotes, and backticks), but the result is the same:
...
We've already explored quite a bit of our JavaScript universe, but there is just one more (quick) stop on the first part of our tour: Symbols. Symbols are a relatively recent addition to the language.
...
It’s important to know that Symbols exist, but it's hard to explain their purpose and behavior without diving deeper into objects and properties. They are also relatively rare, so in our universe, we're going to skip them. Sorry, symbols!


## To Be Continued

This module is split in two parts. We’ve reached the end of the Part 1. We’ll take a small break from our tour now. Let’s recap the primitive values we’ve encountered so far!
...
In the second part, we will continue our sightseeing tour. Specifically, we will look at Objects and Functions.
