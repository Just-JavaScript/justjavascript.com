export const meta = {title: 'Values and Variables', series: 1, episode: 3}
...
<Answer title="Answer" byline="Don’t reveal until you have finished writing.">
...
In my universe, a variable is a wire. It has two ends and a direction: it starts as name in my code and it ends pointing to some value in my universe.

For example, I can point the `pet` variable to the `"Narwhal"` value:
...
It turns out that a variable name like `pet` can serve as an expression too! When we write `pet`, we’re asking JavaScript a question: “What is the current value of `pet`?” To answer our question, JavaScript follows the `pet` “wire”, and gives us back the value at the end of this “wire”.
...
- Declare a variable (a wire) called `y`.
...
At the end, the `x` variable points to the value `0`, and the `y` variable points to the value `10`. Note that `y = x` did not mean point `y` to `x`”. We can’t point variables to each other! **Variables always point to _values_.** When we see an assignment, we “ask” the right side’s value, and point the “wire” on the left side to it.
...
[Click here to solidify this mental model with a few short exercises.](https://eggheadio.typeform.com/to/RWJg3m?email={{subscriber.email_address}})
...
<Pagination next="04-counting-the-values-pt1" prev="02-the-javascript-universe">
