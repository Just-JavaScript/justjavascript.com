...
At the end, the `x` variable points to the value `0`, and the `y` variable points to the value `10`. Note that `y = x` did not mean point `y` to `x`”. We can’t point variables to each other! **Variables always point to _values_.** When we see an assignment, we “ask” the right side’s value, and point the left side’s “wire” at it.
