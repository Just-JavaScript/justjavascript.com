...
In this module, we’ll introduce *prototypes*. Prototypes explain what happens in this puzzle and—more importantly—are at the heart of several other fundamental JavaScript features. Occasionally people neglect learning about prototypes because they seem too unusual, but the core idea is remarkably simple.
...
<Video src="https://res.cloudinary.com/dg3gyk0gu/video/upload/v1614268488/just-javascript-email-images/jj09/proto_anim-mp4.mp4" title="Following prototype" />
...
The `human` variable points to object that has a `teeth` property, so the value of `human.teeth` is `32`. The `gwen` variable points to an object that doesn’t have a `teeth` property. However, that object has a prototype, which *does* have a `teeth` property. This is why the value of `gwen.teeth` is also `32`.
...
At first, this might be a bit mindblowing. Let that sink in. All this time, we were thinking `{}` created an “empty” object, but it’s not so empty after all! It has a hidden `__proto__` wire that points to the Object Prototype by default.
...
You might be wondering: what in the world is the `prototype` property? You might have seen it [in MDN documentation](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter).

The story around this is confusing. Before JavaScript added [classes](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes), it was common to write them as functions that produce objects, for example:
