...
const InviteTeam = ({teamPurchase}) => {
  ...
  return (
    <>
      ...
      <div className="flex max-w-screen-sm w-full">
        <a
          href={teamPurchase.bulk_coupon_url}
          target="_blank"
          aria-label={teamPurchase.bulk_coupon_url}
          className="max-w-screen-sm font-mono overflow-hidden text-xxs rounded-l-lg bg-transparent hover:bg-gray-100 hover:bg-opacity-25 border border-gray-200 text-text flex items-center justify-start p-3 transition-all ease-in-out duration-200 leading-none"
        >
        ...
        >
          <button
            ...
            className="rounded-r-lg -ml-px bg-transparent hover:bg-gray-100 hover:bg-opacity-25 border border-gray-200 text-gray-600 hover:text-text flex items-center justify-center p-3 transition-colors ease-in-out duration-200 leading-none"
            type="button"
          >
            {isCopied ? (
              // prettier-ignore
...
const TeamInvites = ({teamPurchases}) => {
  return (
    <>
      {!isEmpty(teamPurchases) && (
        <div className="mb-20 text-text print:hidden">
